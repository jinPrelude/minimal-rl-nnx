...
class PPOGTrXL(nnx.Module):
    def __init__(
        ...
        *,
        ...
        self.conv1 = nnx.Conv(in_features=1, out_features=32, kernel_size=(8, 8), strides=(4, 4), padding="VALID", rngs=rngs)
        self.conv2 = nnx.Conv(in_features=32, out_features=64, kernel_size=(4, 4), strides=(2, 2), padding="VALID", rngs=rngs)
        self.conv3 = nnx.Conv(in_features=64, out_features=64, kernel_size=(3, 3), strides=(1, 1), padding="VALID", rngs=rngs)
        self.fc_encoder = nnx.Linear(3136, trxl_dim, rngs=rngs)
    ...
    def _encode(self, obs):
        x = jnp.asarray(obs, dtype=jnp.float32) / 255.0
        x = x[..., None]
...
def main():
    ...
    assert args.env_name == "ALE/Breakout-v5", "This minimal implementation supports only ALE/Breakout-v5."
    ...
    memory_length = args.trxl_memory_length

    wandb.init(
        project="minimal-flaxrl",
        name=f"ppo_gtrxl_atari_{args.env_name}",
    ...
    atari_preprocessing = functools.partial(
    ...
    envs = gym.make_vec(
        ...
        wrappers=(atari_preprocessing,),
        frameskip=1,
    )
    envs = gym.wrappers.vector.RecordEpisodeStatistics(envs)

    assert envs.single_observation_space.shape == (84, 84)
    assert envs.single_observation_space.dtype == np.uint8
